...
*/
  ...
  async function fetchy(url) {
    try {
      ...
      console.error("Something went whoopsie \n", error);
    }
  }

  async function fetchyBatchy() {
    const stuff = await Promise.all(
      savedActivityKeys.map((activity) =>
        fetchy(`https://bored.api.lewagon.com/api/activity?key=${activity}`)
      )
    );
    setActivitiesData(stuff);
    // console.log(stuff);
  }
